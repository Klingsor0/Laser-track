...
def render():
    ...
    """
    # Al inicio de tab6_esp32_camera.py, en el selector:
    ...
    source_type = st.radio(
        "Camera Source",
        ["USB Webcam", "ESP32-CAM (WiFi)"],
        horizontal=True
    )

    if source_type == "USB Webcam":
        # USB CAMERA LOGIC
        col1, col2 = st.columns([2, 1])
        
        with col1:
            camera_index = st.selectbox("Camera ID", [0, 1, 2, 3, 4], index=0)
                ...
                try:
                    ...
                    if cap.isOpened():
                        ...
                        if ret:
                            ...
                            if 'esp32_last_frame' not in st.session_state:
                                st.session_state.esp32_last_frame = None
                        else:
                            cap.release()
        ...
        if st.session_state.esp32_connected and st.session_state.esp32_stream is not None:
            ...
            try:
                ...
                if ret:
                    ...
                    st.session_state.esp32_last_frame = frame_rgb.copy()
                    ...
                    if 'esp32_last_frame' in st.session_state and st.session_state.esp32_last_frame is not None:
                        st.info("📷 Last captured frame (camera disconnected)")
                        st.image(
                            st.session_state.esp32_last_frame,
                            caption="Last Frame (Disconnected)",
                            use_container_width=True,
                            channels="RGB"
                        )
                        st.markdown("---")
        ...
        # Rest of setup instructions remain the same...
